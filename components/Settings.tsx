...
import { Profile, Transaction, Project, User, ViewType, ProjectStatusConfig, SubStatusConfig, PublicPageConfig } from '../types';
    ...
    const handleUserFormSubmit = (e: React.FormEvent) => {
        ...
        if (userModalMode === 'add') {
            ...
            const newUser: User = {
                ...
                vendorId: currentUser.vendorId, // Assign to the current admin's vendor
            };
                 ...
                 return (
                    <form onSubmit={handleSubmit}>
                        <div className="space-y-6">
                            ...
                            <h3 className="text-lg font-semibold text-brand-text-light border-b border-gray-700/50 pb-3 mt-8">Manajemen Data</h3>
                            <div className="space-y-4 max-w-2xl mx-auto">
                                <div className="flex justify-between items-center p-4 bg-brand-bg rounded-lg">
                                    <div>
                                        <p className="font-semibold text-brand-text-light">Reset Data Aplikasi</p>
                                        <p className="text-sm text-brand-text-secondary">Kembalikan semua data ke kondisi awal. Semua perubahan akan hilang.</p>
                                    </div>
                                    <button
                                        type="button"
                                        onClick={() => {
                                            if (window.confirm('APAKAH ANDA YAKIN? Semua data klien, proyek, dan keuangan akan dihapus dan dikembalikan ke data demo. Tindakan ini tidak dapat diurungkan.')) {
                                                localStorage.clear();
                                                window.location.reload();
                                            }
                                        }}
                                        className="button-secondary !bg-brand-danger/10 !border-brand-danger/20 !text-brand-danger hover:!bg-brand-danger/20"
                                    >
                                        Reset Data
                                    </button>
                                </div>
                    ...
                    {userForm.role === 'Member' && (
                        <div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 p-3 bg-brand-bg rounded-lg">
                                {NAV_ITEMS.filter(item => item.view !== ViewType.DASHBOARD && item.view !== ViewType.SETTINGS && item.view !== ViewType.SUBSCRIPTION_DETAILS).map(item => (
